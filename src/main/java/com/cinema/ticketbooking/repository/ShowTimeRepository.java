...
public interface ShowTimeRepository extends JpaRepository<ShowTime, Long>, JpaSpecificationExecutor<ShowTime> {
    @org.springframework.data.jpa.repository.Query("select s from ShowTime s join fetch s.film left join fetch s.auditorium where (:date is null or s.date = :date) and s.startTime is not null and (s.date > :today or (s.date = :today and s.startTime > :time)) order by s.date, s.startTime, s.id")
